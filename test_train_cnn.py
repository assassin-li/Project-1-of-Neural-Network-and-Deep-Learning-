...
# reshape to [batch, channel, H, W] for CNN
train_imgs = train_imgs.reshape(-1, 1, 28, 28)
valid_imgs = valid_imgs.reshape(-1, 1, 28, 28)

cnn_model = nn.models.Model_CNN(lambda_list=[1e-4, 1e-4, 1e-4, 1e-4])

test_output = cnn_model(valid_imgs[:5])
print("Initial logits range:", test_output.min(), test_output.max())
print("Initial logits:", test_output[0])
...
runner.train([train_imgs, train_labs], [valid_imgs, valid_labs], num_epochs=5, log_iters=100, save_dir=r'./best_models_cnn')
